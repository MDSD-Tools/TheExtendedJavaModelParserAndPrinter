...
public class CentralBindingBasedResolver {
	...
	Resource findPackageResourceInResourceSet(String packageName) {
		for (Resource res: resSet.getResources()) {
			if (res.getContents().size() > 0) {
				if (res.getContents().get(0) instanceof tools.mdsd.jamopp.model.java.containers.Package) {
					tools.mdsd.jamopp.model.java.containers.Package pack =
							(tools.mdsd.jamopp.model.java.containers.Package) res.getContents().get(0);
	...
	Resource findModuleResourceInResourceSet(String moduleName) {
		for (Resource res : resSet.getResources()) {
			if (res.getContents().size() > 0) {
				if (res.getContents().get(0) instanceof tools.mdsd.jamopp.model.java.containers.Module) {
					tools.mdsd.jamopp.model.java.containers.Module mod =
							(tools.mdsd.jamopp.model.java.containers.Module) res.getContents().get(0);
