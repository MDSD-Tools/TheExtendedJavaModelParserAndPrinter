...
public class ConcreteClassifierDecider extends AbstractDecider {
	...
	private String packageName(IdentifierReference p) {
		...
		while (p != null) {
			if (p.getTarget() instanceof tools.mdsd.jamopp.model.java.containers.Package) {
				s = ((tools.mdsd.jamopp.model.java.containers.Package) p.getTarget()).getNamespacesAsString();
				break;
